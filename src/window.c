...
int window_level(uint32_t wid)
{
    ...
    if (workspace_is_macos_ventura() || workspace_is_macos_sonoma() || workspace_is_macos_sequoia() || workspace_is_macos_tahoe() || workspace_is_macos_golden_gate()) {
        CFArrayRef window_ref = cfarray_of_cfnumbers(&wid, sizeof(uint32_t), 1, kCFNumberSInt32Type);
...
static int SLSGetWindowSubLevel__Internal(int cid, uint32_t wid)
{
    ...
    msg.header.msgh_id = (workspace_is_macos_tahoe() || workspace_is_macos_golden_gate()) ? 0x76E3 : 0x73C3;
